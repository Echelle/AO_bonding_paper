...
We also used IR imaging to inspect VG09-12 and VG15-13.  We detected IR bubbles in all 3 of these bonded Si samples.  The observable bubble areal density varied from about 4 per 100 mm diameter bonded wafer pair to over 20 per bonded wafer pair.  The individual IR bubble areas were as small as a few square millimeters up to $400\;\mathrm{mm}^2$.  The IR bubbles had varied morphologies.  The structure within the IR bubbles was sometimes indicative of Newton's rings, exposing the presence of gaps larger than about $\lambda/2$ up to $\sim 15 \lambda$.  As expected, the unbonded DSP wafers show no such IR bubbles.  No attempt at quantitative measurement was made on the IR images due to evidence of a non-linear detector response.  The bulk morphologies were clear regardless of the detector response.  When IR imaging was available, it was leveraged to target IR specectroscopy in locations demonstrating presence or absense of IR bubbles, as desired.  In other cases when IR imaging was not available or ignored, spectra were taken at random positions on the wafer.
...
where $\delta_{ij}$ is the Kronecker delta, and $\sigma_i$ are the independent measurement uncertainties on the $i^{\mathrm{th}}$ data point. We experimented with different values for $\sigma_i$.  When repeated DSP reference sample measurements were available, we estimated the $\sigma_i$ from the dispersion around mean-subtracted, detrended transmission spectra.  The parameter $s$ controls the correlation length, and $a$ controls the amplitude of the noise.  We do not know $a$ nor $s$ and they are, in general, different for each measurement.  The value of $a$ should be close to the mean drift reported in Table \ref{tabCary5000pars}.  The value of $s$ could be small (tens of nm) to capture local variation attributable to atmospheric air absorption, or large (hundreds or thousands of nm) to capture global features attributable to lamp drift.  We allow both $a$ and $s$ to be free parameters in our fitting procedure.
...
where the residual vector $\boldsymbol{r}$ is the data $T_{obs}$ minus the model $T_{mix}$.  We produce posterior samples with Markov Chain Monte Carlo (MCMC).  Specifically, we implemented \texttt{emcee}\footnote{\url{https://github.com/dfm/emcee}}\cite{emcee}, with 32 walkers, hundreds of burn-in samples, and 600-1000 iterations.  We initialized the walkers in the vicinity of our best guess for parameters based on visual inspection of the spectra.  Figures \ref{figVG03full}, \ref{figVG03part}, and \ref{figVG12_corner} show examples of posterior samples including corner plots of the interesting physical parameters $d$ and $f$.

\subsection{Spectra of direct bonded Si with known gap sizes}
\label{secKnownGaps}

Figure \ref{figVG03full} shows a parameter determination for the measurement of a bond over a deep ($4000 \pm 200\;$nm) 100\% fill factor gap.  The analysis yields a best fit consistent with 100\% fill factor, and a depth of $3960 \pm 2\;$nm, well within the uncertainty of the physicsal measurement.  Figure \ref{figVG03part} shows a fit for a region of the same bonded pair where a nominally $4000\;$nm deep, 2 mm wide trench crosses the field scanned by the Cary 5000.  Analysis of this transmission curve gave a best fit depth of $4094 \pm 4\;$nm and a pattern fill factor of $4.6 \pm 0.1\;\%$, both consistent with the part geometry.  These two results show that the formalism performs as expected on resonant gaps several wavelengths deep.
...
\begin{equation}
	T_{mix} =  0.5 \; T_{e}(d_M + \epsilon) + 0.5 \; T_{e}(\epsilon) \label{eqnMix2Gaps}
\end{equation}
...
We acquired spatially resolved measurements of sample VG09-12 with the Cary 5000 sample transport accessory\verb|\ref{link-to-STA-website}|.  This accessory translates the sample through the measurement beam and takes a transmission spectrum at each position.  The 2 mm step size is comparable to the monochromator's slit size.  We sampled 50 mm across the mesh pattern and into the off-mesh pattern.  The large number of consecutive measurements prevented the requisite near-contemporaneous baseline measurements for normalization.  We fit a line to before-and-after baseline measurements to infer the mean-level drift, which we divided out of each spectrum.  We reduced the measured wavelength range to $1250-1775\;$nm.

%Figure \verb|\ref{photo-of-VG09-12}| shows an illustration of sample VG09-12, with the approximate center locations of measurements marked with colored dots.  The colored dots match the color of the spectra in Figure \verb|\ref{fig-STA-obs}| The approximate size of the measurement beam is shown for scale.  The conspicuous mesh pattern has a lab measured depth of $49\pm6\;$nm, and fill factor of 50\%.  The prediction band for a transmision spectrum with those properties, and zero extra gap is shown as the green apple colored swath.  If the mesh pattern were perfectly bonded, we would expect all the spectra to fall inside the color swath in Figure \verb|\ref{fig-STA-obs}|.  Instead we see that some spectra are inside the swath, and others show a deficit of flux, indicating an extra gap as in the mixture model in Equation \verb|\ref{mixture-model-equation}|.

Sample VG09-12 has a single coarse mesh.  The mesh is composed of boxes plasma-etched to a depth of $49\pm6\;$nm.   The boxes have an edge length of 1.5 mm.  The edges of adjacent boxes are spaced 0.62 mm apart, to yield an overall fill factor of 50\%.  The Cary5000 measurement beam size is $\sim$1 mm $\times$ 10 mm.  Since the beam size is comparable to the mesh size, the fill factor at the position of the measurement will will differ from 50\%. The slit measurement was aligned with the mesh grid.  The highest fill factor acheivable is about 75\%, assuming the slit fit perfectly within a column of boxes.  The slit is larger than 0.62 mm, so the measurement cannot exhibit a 0\% fill factor of voids.  The precise value of the minimum fill factor depends on the details of the slit width, which is not known precisely.  We assume that a minimum fill factor of 30\%, but caution that this boundary is fuzzy.  Figure \verb|\ref{VG09-12 coarse mesh STA}| shows
...
\begin{figure}[htbp]
        ...
        \begin{subfigure}[b]{0.35\textwidth}
              ...
              \caption{Illustration of VG09-12 sample transport accessory measurement positions}
		\label{figVG12_STA_corner}
        \end{subfigure}
        \begin{subfigure}[b]{0.65\textwidth}
                \includegraphics[width=\textwidth]{figs/VG0912_STA_scan_coarse.pdf}
                \caption{Measured gap transmission spectra of VG09-12 on- and off- mesh}
                \label{figVG12_f50}
        \end{subfigure}
\caption{Predictions and measurements of the coarse mesh region in sample VG09-12.
\label{figVG12}}
\end{figure}
...
We devised two experiments to approach the question, ``What is the smallest gap this technique can detect?''.  We applied our MCMC model fitting formalism to an unbonded DSP wafer.  The measured transmission spectrum was normalized by a transmission spectrum of the same sample taken only a few minutes earlier.  Any difference of this normalized transmission from unity is attributable entirely to measurement uncertainty, and represents the smallest possible signal one could extract.  We also measured bonded sample VG09-12 in a region away from the intentionally implanted gap.  This spectrum was processed in the same way as the spectra of the VG09-12 mesh area, exhibiting a mean value of 0.9998 and a standard deviation $\sigma=0.0008$.  This spectrum is consistent with no gap.  We applied our MCMC formalism to further constrain the range of gap sizes consistent with the spectrum.  Table \verb|\ref{tbl summary of gap fit extents}| summarizes the sizes of gaps.  The well-calibrated DSP noise spectrum rules out ($>3\sigma$) gap sizes larger than about 8.0 nm over $>80$\% of the measurement area.  Similarly, the VG09-12 off-mesh spectrum rules out gaps greater than 13 nm over $>80$\% of the measurement area.

The variance of individual measurement uncertainties was large enough to consider for this measurement.  So rather than assume homoscedastic uncertainty as we did for the other measurements, we estimated the heteroscedastic uncertainties in the following way.  We computed the mean-subtracted transmission for four DSP measurements bookending the those of VG02.  The DSP measurements were detrended, and the measurement-to-measurement variance computed.  The result was a variance spectrum which peaks at $1850\;$nm at 0.25\%, but with a median value equal to 0.03\%.  Figure \verb|\ref{fig_xxx}| includes the $1\;\sigma$ errorbars computed in this way.

\begin{figure}[htbp]
        \centering
        \begin{subfigure}[b]{0.5\textwidth}
              \includegraphics[width=\textwidth]{figs/VG02p3_corner.pdf}
              \caption{Fitted gap size $d$ and fill factor $f$ for VG02}
		\label{fig_VG02p3_corner}
        \end{subfigure}

        \begin{subfigure}[b]{0.5\textwidth}
                \includegraphics[width=\textwidth]{figs/VG02_noGap.pdf}
                \caption{Measured spectrum and fit of VG02}
                \label{figVG02_noGap}
        \end{subfigure}
\caption{ Similar to Figure \ref{figVG03full} and \ref{figVG03part}, but for sample VG02, which has no intentionally imbedded gap.  The measured spectrum in this region shows no appreciable deficit of transmission from the gapless counterparts.
\label{figVG12}}
\end{figure}

Figure \ref{fig_VG02p3_corner} shows the degeneracy between the axial extent of a gap and its fractional fill factor over the measurement beam.  Simply put, a larger gap can be hidden if it doesn't cover much area.  The red dashed line shows the normalized transmission spectrum of an 18 nm gap over 30\% of the measurement area.  Gaps larger than 60 nm would have to cover less than 10\% of the measurement area in order to be consistent with the measurement.  On this zoomed-in vertical scale, the individual draws from the Gaussian process model show morphologies consistent with the correlated measurement error (Figure \ref{figVG02_noGap}).


\section{Discussion}

\subsection{Questioning assumptions of the model}

\emph{More complex underlying gap size distributions-} One key assumption of our model is that the distribution of gap sizes over our measurement area is bimodal.  Specifically, Equation \ref{eqnMix} assumes that there is either a gap of size $d$, or else it is 100\% bonded.  If there is a gradient across the bond, the emergent transmission spectrum will be an admixture of many gaps with various areal coverage fractions.  The full solution becomes an areal integral of Equation \ref{eqFP} normalized by the total measurement area.  Solving for the relative distributions of gaps is highly degenerate, in the same way as we saw in the 2-parameter corner plots of gap depth and fill factor.  We experimented with by-eye fitting parametric distributions of gap sizes, characterized by a maximum and minimum gap size, and a monotonic slope of the distribution.  We found that measurements of very large ($>10\lambda$) gaps with a large gradient offered surprisingly strong constraints on the maximum gap size, because of the conspicuous high frequency Fabry-P\`erot fringes.  For sub-wavelength gaps, the gap size distibution is too degenerate to derive much useful insight on the slope of the distribution.  Specifically, for $d_{max}<\lambda/4$, our simple mixture model will converge on the spectrum of the average gap size, even if the \emph{True} distribution is more complex.  Reducing the measurement area could break some of these degeneracies, if more precision is desired.

\emph{Origin and properties of correlated noise- } Another key assumption of the model is that correlated errors dominate the measurement uncertainty.  This is the same thing as saying that there is an instrumental bias, or that our measurements are \emph{precise} but \emph{inaccurate}.  The extent to which the measurement is inaccurate is tabulated in \ref{tbl_cary5000Pars}, as already mentioned.  We have assumed that the Gaussian Process model can capture the effect of the observed correlated errors.  But to what extent does the Gaussian Process emulate the \emph{True} covariance matrix, which is unknown?  One way to gauge the goodness of fit of the covariance matrix is to generate synthetic data from a best-fit model.  Many of these synthetic draws are shown in the spectra in Figures \verb|\ref{a thru f}| as transparent blue lines.  When overplotted these lines look like a halo of uncertainty enveloping the model and measurement.  The bulk agreement is encouraging.  We also scrutinized individual synthethic spectra with the measured data, and found that the noise model tended to under-predict the length scale of the correlation.  For example, we typically derive $0.15<a<0.4\%$ and $20<s<200$.  The measured data appear to prefer larger values of $s$.  The discrepancy probably comes from the fact that, in reality, the \emph{True} covariance matrix probably has local structures that cannot be replicated by a simple 2-parameter (\emph{i.e.} $a$ and $s$) estimate of the covariance matrix.  A model employing local covariance structures \verb|\cite{Czekala et al. in press}| could better capture some of the residual structure, but would introduce additional computational complexity with little gain.  Overall the Gaussian Process technique provides much better agreement than the na\"{i}ve assumption of zero correlated errors.

\emph{Validating the MCMC with synthetic data, and addressing heteroscedasticity-}  To evaluate our MCMC fitting method and assumption of homoscedasticity, we generated synthetic data with a known parameters, $d$, $f$, $a$, and $s$ and $\sigma_i$.  We then fit the parameters $d$, $f$, $a$, and $s$ assuming homoscedastic errors, ($\sigma_i = \sigma = 0.02\%$).  The derived physical parameters were consistent with the \emph{Truth} to within the $1\;\sigma$ confidence interval, but we significantly underestimated $s$ (8 nm instead of 200 nm) and marginally underestimated $a$ (0.0013 instead of 0.0016).  This result is reassuring for our estimation of physical parameters, and shows that the Gaussian Process technique captures realistic uncertainty in the physical parameters, despite an imperfect characterization of the noise correlation length.

\emph{Polarization and lamp jumps- } As mentioned, the Cary 5000 double beam mode offers considerably lower amplitude of correlated errors than single beam mode \ref{tbl_cary5000Pars}.  However, the double beam mode was found to have enhanced artifacts at wavelengths at which the Cary 5000 monochromator changes its diffraction grating and light sources.  The sharp jumps in transmission were up to a few percent.  The origin of these jumps could be polarization-dependent transmission losses.  Whatever the origin, they cause large systematic errors.  These are still captured by the Gaussian Process structure.
...
\emph{DSP vs AR coated front faces- } Another key assumption of the model is that all the Si$-$air interfaces are well-approximated by Fresnel reflection losses.  If the entrance and exit faces of the bonded samples are AR coated, the incoherent multiple reflection model will have to be modified.  AR coated front and exit faces will result in a larger interface loss for a given gap size than the interface loss computed here.


\emph{Refactive index- } We have assumed that the refractive index is identical to that given by \cite{2006SPIE.6273E..77F}.  Double beam mode measurements can reach a precision of comparable to the part-to-part variation of refractive index.  We have normalized spectra by their singleton counterparts from the same heritage (i.e. same wafer box).

\emph{Diffraction from mesh patterns}
It's conceivable that the mesh patterns Figure \ref{figVS20pattern}.  We ruled out wavelength-dependent diffraction from the mesh pattern as a measurement artifact, since the beam size is large and the angular spread of diffraction from the mesh pattern would be much smaller than the spread of the beam.  To eliminate this concern, we measured in the coarse mesh rather than the medium or fine meshes, when possible.  We saw no difference in the bond quality surrounding the medium or fine meshes, which is consistent with results from the literature \cite{1992JEMat..21..669M}.  Specifically, the gas pressure of bonded wafers grows as the fill factor of gaps decreased.  In other words, what matters is the fill factor of gaps, not their relative sizes.  Since the fine, medium, and coarse meshes had the same fill factors, we anticipate no difference in the pressure of the gaps, and therefore no difference in the bonding properties.

\subsection{What is the smallest gap size this technique can measure?}
In this section we quantify the limits of how far the technique can be pushed.  There are three key ideas when trying to answer the question ``What is the smallest gap size this technique can measure?''.  The answer for single beam mode is evidently $\sim20 nm$ (Figure \ref{fig_VG02p3_corner}).  In principle this number could be pushed down by averaging many spectra, all normalized by different reference spectra.  Alternatively, double beam mode could be used.  The grating jumps noted above would also limit double beam mode analysis.  If these could be rectified, a very low uncertainty would be obtained, and it is conceivable that gaps as small as a few nanometers could be inferred.
