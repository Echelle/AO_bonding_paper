...
Our sensitive measurement technique introduces a few challenges at the parts per thousand level baseline level.  Specifically, we have been ambivalent about whether or not to use the double-beam capability of the Cary5000.  We have shown that this mode reduces by a factor of 10, the random flicker attributable to lamp noise.  In practice, the double beam mode can cause some systematic problems near the portions of wavelengths near lamp and filter changes.  So of these issues might be related to polarization differences, and a way to mitigate these techniques is to follow the advice of the vendor and employ a depolarizer in the beam path.  A depolarizer strategy is critical for applications in which a non-normal incidence angle is desired.  A half-hearted attempt to measure non-normal incidence without the polarization accessory resulted in failure.  In the next section we describe our interest in non-normal incidence.

\subsection{Non-normal incidence angles}
For many applications, normal incidence performance is the quantity of interest.  The technique described here outlines an approach for measuring the gap size of bonded Si wafers at normal incidence.  In principle, from there the performance at non-normal incidence can be worked out either by simulation or calculation.  Here we briefly motivate the challenge of experiment and simulation at non-normal incidence.  

Our final application that motivated this project is to make diffraction gratings on the hypotenuse of a silicon prism.  The diffraction occurs inside the silicon material and is dispersed and reflected back along its path.  This configuration is called an immersion grating.  Our group has made monolithic immersion gratings for over a decade, but challenges in processing large (30 mm thick) Si wafers motivated an approach of patterning and processing thin (1-10 mm thick) Si wafers, and then bonding them to shaped and polished prisms.  This technique has been pioneered at SRON.

The consequence of our design is a non-normal incidence interface in double pass.  In this case, the framework developed here fails to predict the spectrum of the interface loss attributable to a finite albeit small (few nm) gap.  Specifically, at large incidence angles complete internal reflection and evanescent waves will matter.  Polarization effects are important.      
